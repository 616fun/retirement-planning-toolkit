...
```bash
git clone <your-fork-url> retirement-planning-toolkit
cd retirement-planning-toolkit
...
| company-health | `yfinance`, `edgartools` | live employer-stock price, analyst, and SEC-filing signals (optional) |

**No API keys are required.** All data comes from free public sources (Yahoo
Finance, SEC EDGAR). EDGAR only needs a contact identity string you set in your
config (`employer_stock.sec_identity`, e.g. `"Your Name you@example.com"`).
...
| `Can't find engine/ next to setup.py` | Run from the wrong folder | `cd` into the cloned repo (the folder with this file), then re-run. `setup.py` itself is now path-independent. |
| `company_health.py` shows "EDGAR unavailable" | `edgartools` not installed, or no SEC identity | `python3 setup.py --yes`, and set `employer_stock.sec_identity` in your config to `"Your Name you@example.com"`. |
| Price/analyst fields show `n/a` | No internet, or Yahoo Finance throttled | Check your connection and retry. The rest of the toolkit still runs offline. |
...
those are staged. Never commit statements, tax docs, or credentials.
