...
Resolution order:
  ...
  4. config/examples/rivera_config.json  (fictional demo fallback)
"""
...
def _candidate_paths(explicit=None):
    ...
    yield ROOT / "config" / "examples" / "rivera_config.json"
...
def load_config(path=None, validate=True):
    ...
    raise FileNotFoundError(
        ...
        "RPT_CONFIG=config/examples/rivera_config.json."
    )


# Keys the engine dereferences directly (no .get default), grouped by section.
_REQUIRED = {
    "household": ["name", "members", "state",
                  "state_income_tax_rate", "local_income_tax_rate"],
    ...
    "social_security": ["spouse_a_monthly_benefit", "spouse_b_monthly_benefit"],
    "assumptions": ["portfolio_return_base", "portfolio_return_conservative",
                    ...
                    "bridge_target", "standard_deduction_mfj", "irmaa_tier1_magi_mfj"],
...
def validate_config(cfg):
    ...
    for k in ("spouse_a_monthly_benefit", "spouse_b_monthly_benefit"):
        v = cfg.get("social_security", {}).get(k)
        if v is not None and not _is_number(v):
            errors.append(f"social_security.{k} must be a number (got {v!r})")
    for k in ("state_income_tax_rate", "local_income_tax_rate"):
        v = cfg.get("household", {}).get(k)
        if v is not None and not _is_number(v):
...
def investable_total(cfg):
    """Sum of all liquid/retirement accounts (excludes real estate and 529s)."""
    acct = cfg["accounts"]
    exclude = {"college_529_a", "college_529_b"}
